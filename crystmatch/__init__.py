...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage = "%(prog)s [-h] [-V] [-E POSCAR_I POSCAR_F MAX_MU MAX_W] [-R CSMLIST [IND1 IND2 ...]] [-D POSCAR_I POSCAR_F] "\
    ...
    parser.add_argument("-E", "--enumerate", nargs=4, type=str, metavar=('POSCAR_I', 'POSCAR_F', 'MAX_MU', 'MAX_W'),
                        help="enumerate representative CSMs between POSCAR_I and POSCAR_F, with MAX_MU and MAX_W as upper bounds for the multiplicity and RMSS")
    ...
    if mode == 'enumerate':
        ...
        max_w = float(args.enumerate[3])

        if max_mu < 1: raise ValueError("Multiplicity should be a positive integer.")
        elif max_w <= 0: raise ValueError("Root-mean-square-strain should be a positive float.")
        if max_mu >= 8 or max_w > 0.5:
            print(f"Warning: Current MAX_MU = {max_mu:d} and MAX_RMSS = {max_w:.2f} may result in a large number of CSMs, which may take "
        ...
        is_stoichiometric(crystA[1], crystB[1])
        job = f"m{max_mu:d}s{max_w:.2f}"
        csm_bins, slmlist, mulist, rmsslist, rmsdlist = enumerate_representative(crystA, crystB, max_mu, max_w, args.tolerance)
        ...
        job, crystA, crystB, max_mu, max_w, symprec = data['metadata']
        ...
        hA, qA = hnf(mA, return_q=True)
        hB, qB = hnf(mB, return_q=True)
    ...
    if args.enumerate is not None:
        ...
        metadata = np.array([job, crystA, crystB, max_mu, max_w, args.tolerance], dtype=object)
