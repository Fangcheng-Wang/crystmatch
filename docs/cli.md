...
crystmatch [--help] [--version]
           ...
           [--direct POSCAR_I POSCAR_F] [--nebmake IMAGES] [--restore]
           [--all MAX_D]
           [--interact [SIZE]]
...
- `-r`, `--restore`
    ...
    Add integers to the fractional coordinates in `POSCAR_F` when using `--direct` to minimize the shuffle distance. This option is useful when you want to **restore** the original fractional coordinates of the final structure.

    !!! warning "Caveat"
        If you are reading a pair of POSCAR files generated by `crystmatch`, you should not use this option, as `--restore` will alter the CSM.
